...
resource "aws_api_gateway_rest_api" "flooded" {
  name = "flooded"
  description = "Flooded API demo"
}

resource "aws_api_gateway_method" "root_get" {
  rest_api_id = "${aws_api_gateway_rest_api.flooded.id}"
  resource_id = "${aws_api_gateway_rest_api.flooded.root_resource_id}"
  http_method = "GET"
  authorization = "NONE"
}
...
resource "aws_api_gateway_resource" "oauth" {
  rest_api_id = "${aws_api_gateway_rest_api.flooded.id}"
  parent_id = "${aws_api_gateway_rest_api.flooded.root_resource_id}"
  path_part = "oauth"
}
...
resource "aws_api_gateway_method_response" "oauth_delete_200" {
  rest_api_id = "${aws_api_gateway_rest_api.flooded.id}"
  resource_id = "${aws_api_gateway_resource.oauth.id}"
  http_method = "${aws_api_gateway_methodflooded_response.oauth_delete_200.status_code}"
}
...
output "dns_name" {
  value = "${aws_api_gateway_rest_api.flooded.id}.execute-api.us-west-2.amazonaws.com"
}
